...
const ChatInterface = () => {
  ...
  // On initial mount, load saved conversation history from backend
  useEffect(() => {
      ...
      try {
        const response = await fetch(`${backendUrl}/api/chat/history`, {
          ...
          headers: {
            ...
            token, // Ensure backend uses this for authentication
          },
        });
        ...
        if (data.conversations) {
          // Build history and titles objects keyed by conversationId
          const history = {};
          const titles = {};
          data.conversations.forEach((conv) => {
            history[conv.conversationId] = conv.messages;
            // For simplicity, use a default title using the conversationId or any custom logic
            titles[conv.conversationId] = `Chat ${conv.conversationId}`;
          });
  ...
  const sendToServer = async (message) => {
    try {
      ...
      if (data.error) {
        ...
        // Update active conversation ID if backend returns one
        if (!activeConversationId && data.conversationId) {
          setActiveConversationId(data.conversationId);
          // Also update conversationTitles state so this new chat shows in the sidebar
  ...
  // Create a new chat, and save the previous chat to history
  ...
  const handleChatHistoryClick = (id) => {
    // Save current messages to the active conversation history
